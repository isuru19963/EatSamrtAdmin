    ...
    public function index()
    {
        if (tenant('domains') == null) {
            if (!file_exists(storage_path() . "/installed")) {
                ...
                \Log::info('new');
                ScheduleRun::runTimeChallenges();
                $now = date("Y-m-d h:i:s", strtotime("now"));
                $lastdate= date('Y-m-d h:i:s',(strtotime ( '-7 day' , strtotime ( $now) ) ));

                $user = User::where('type', 'User')->whereBetween('created_at', [$lastdate,$now ])->count();
                $total_user = User::where('type', 'User')->count();
                $ios_users = User::where('type', 'User')->where('device_type', 'IOS')->count();
                $android_users =  User::where('type', 'User')->where('device_type', 'Android')->count();
                $plan = Plan::count();
                $languages = count(UtilityFacades::languages());
                $earning = Order::sum('amount');


                return view('dashboard.home', compact('user', 'total_user', 'ios_users', 'android_users','plan','languages','earning'));
            }
        } else {
